...
beforeEach(() => {
  ...
  vi.mocked(accountApi.requestRegistration).mockResolvedValue(challenge);
  vi.mocked(accountApi.requestAction).mockResolvedValue(challenge);
});
...
describe('Public authentication', () => {
  ...
  it('provides registration without embedded demo credentials', () => {
    ...
    expect(
      screen.queryByText(/demonstration accounts/i),
  ...
  it('requires matching passwords before requesting an email', () => {
    ...
    fireEvent.submit(
      screen
        .getByRole('button', {
          name: 'Continue with email verification',
        })
        .closest('form')!,
    );
    ...
    expect(
      accountApi.requestRegistration,
  ...
  it('does not log in before a valid code and preserves errors for correction', async () => {
    ...
    vi.mocked(accountApi.verifyRegistration).mockRejectedValueOnce(
    ...
    ).toHaveTextContent('Incorrect code');

    expect(auth.login).not.toHaveBeenCalled();

    vi.mocked(accountApi.verifyRegistration).mockResolvedValueOnce({
...
describe('Account security', () => {
  ...
  it('changes password only after code verification and signs out', async () => {
    ...
    expect(accountApi.confirmAction).not.toHaveBeenCalled();

    vi.mocked(accountApi.confirmAction).mockResolvedValue({
    ...
    expect(accountApi.confirmAction).toHaveBeenCalledWith(
      'password',
      'challenge',
      '123456',
    );
