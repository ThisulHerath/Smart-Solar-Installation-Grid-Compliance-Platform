...
export function EmailCodeForm({ challenge, busy, onVerify, onResend, onBack, label = 'Verify email' }: {
  ...
  return <div className="account-form"><p role="status">We sent a six-digit code to <strong>{challenge.maskedEmail}</strong>. Check your inbox and spam folder.</p>
    <ValidatedForm className="account-form" onSubmit={event => { event.preventDefault(); onVerify(code); }}><label htmlFor="email-code">Email verification code</label>
      ...
      <button className="btn btn-primary" disabled={busy || expired}>{busy ? 'Please wait…' : label}</button></ValidatedForm>
